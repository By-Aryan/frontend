...
const AllBuyersDataTable = ({ role, agents, isLoading }) => {
  const [status, setStatus] = useState(null);
  const [id, setId] = useState(null);
  const [message, setMessage] = useState("Buyer deleted Successfully");
  ...
  const getUserProfileImage = (buyer) => {
    return getUserImageUrl(buyer);
  };
  ...
  const handleAgentDeleteClick = async (id) => {
    deleteAgentMutation.mutate(id, {
      ...
      onError: (error) => {
        setStatus(false);
        setMessage("Unable to delete Buyer");
        setState((prev) => ({ ...prev, open: true }));
        setTimeout(() => {
          setState((prev) => ({ ...prev, open: false }));
        }, 3000);
      ...
      <table className="table-style3 table at-savesearch" style={{ fontSize: '14px' }}>
        ...
        <tbody className="t-body">
          {agents && agents.length > 0 ? (
            agents.map((buyer) => (
              <tr key={buyer._id} style={{ lineHeight: '1.2' }}>
                <th scope="row" className="py-2">
                  <div className="listing-style1 dashboard-style d-xxl-flex align-items-center mb-0">
                    <div className="list-thumb" style={{ width: '70px', height: '70px', borderRadius: '8px', overflow: 'hidden' }}>
                      {getUserProfileImage(buyer) ? (
                        <Image
                          width={70}
                          height={70}
                          className="w-100 h-100 object-cover"
                          src={getUserProfileImage(buyer)}
                          alt={buyer.fullname || "Buyer"}
                          unoptimized={buyer.profile_photo?.startsWith('blob:') || buyer.profile_photo?.startsWith('data:')}
                        />
                      ) : (
                    ...
                    <div className="list-content py-0 p-0 mt-1 mt-xxl-0 ps-xxl-4">
                      <div className="h6 list-title mb-1">{buyer.fullname || "Unnamed Buyer"}</div>
                      <span className={getStatusStyle(buyer.status || "active")}>
                        {buyer.status || "active"}
                      </span>
                    </div>
                  </div>
                ...
                <td className="vam py-2">
                  <p className="list-text mb-1">{buyer.email || "N/A"}</p>
                  <p className="list-text mb-0">{buyer.mobile || "N/A"}</p>
                </td>
                <td className="vam py-2">{buyer.role || "N/A"}</td>
                <td className="vam py-2">{formatDate(buyer.createdAt)}</td>
                <td className="vam py-2">
                  <div className="d-flex gap-2">
                    <Link
                      href={`/dashboard/admin/edit-buyer/${buyer._id}`}
                      className="icon"
                      style={{ border: "none" }}
                      data-tooltip-id={`edit-${buyer._id}`}
                    >
                    ...
                    <button
                      className="icon"
                      style={{ border: "none" }}
                      data-tooltip-id={`delete-${buyer._id}`}
                      onClick={() => {
                        handleAgentDeleteClick(buyer._id);
                      }}
                    >
