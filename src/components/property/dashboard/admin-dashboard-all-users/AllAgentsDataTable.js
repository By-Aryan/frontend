...
const AllAgentsDataTable = ({ role, agents, isLoading }) => {
  ...
  const getUserProfileImage = (agent) => {
    return getUserImageUrl(agent);
  };
        ...
        <tbody className="t-body">
          {agentsData && agentsData.length > 0 ? (
            agentsData.map((agent) => (
              <tr key={agent._id} style={{ lineHeight: '1.2' }}>
                <th scope="row" className="py-2">
                  <div className="listing-style1 dashboard-style d-xxl-flex align-items-center mb-0">
                    <div className="list-thumb" style={{ width: '70px', height: '70px', borderRadius: '8px', overflow: 'hidden' }}>
                      {getUserProfileImage(agent) ? (
                        <Image
                          ...
                          src={getUserProfileImage(agent)}
                          alt={agent.fullname || "Agent"}
                          unoptimized={agent.profile_photo?.startsWith('blob:') || agent.profile_photo?.startsWith('data:')}
                        />
                      ) : (
                    ...
                    <div className="list-content py-0 p-0 mt-1 mt-xxl-0 ps-xxl-4">
                      <div className="h6 list-title mb-1">{agent.fullname || "Unnamed Agent"}</div>
                      <span className={getStatusStyle(agent.status || "active")}>
                        {agent.status || "active"}
                      </span>
                    </div>
                  </div>
                ...
                <td className="vam py-2">
                  <p className="list-text mb-1">{agent.email || "N/A"}</p>
                  <p className="list-text mb-0">{agent.mobile || "N/A"}</p>
                </td>
                <td className="vam py-2">{agent.role || "N/A"}</td>
                <td className="vam py-2">{formatDate(agent.createdAt)}</td>
                <td className="vam py-2">
                  <div className="d-flex gap-2">
                    <Link
                      href={`/dashboard/admin/edit-agent/${agent._id}`}
                      className="icon"
                      style={{ border: "none" }}
