...
const VideoOptionField = ({ setVideos, clearVideos = false }) => {
  const [uploaded, setUploaded] = useState(false);
  ...
  const handleSave = () => {
    setVideos(uploadedVideos.map((file) => file.filename)); // store filenames only
    setUploaded(true);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <p className="text fz-10 mb10">Videos must be less than 25MB</p>
        <label
          className={`ud-btn btn-white ${
