...
const RentalPricing = ({ rentalPlans, propertyType }) => {
  ...
  // Fetch property details to determine if it's rental or sale
  const { data: propertyData, isLoading: propertyLoading } = useAxiosFetch(
    propertyId ? `/property/propertyById/${propertyId}` : null
  );
  
  // Determine if property is rental
  const isRentalProperty = propertyData?.data?.details?.purpose === "Rent" || 
                          (propertyData?.data?.details?.purpose && 
                           propertyData.data.details.purpose.toLowerCase().includes("rent"));
  


  // Same features for all properties - no rental-specific differences
  const getPlanFeatures = (planName) => {
    const features = {
      "Basic Boost Plan": [
        "Featured in rental search results",
        "Priority listing for 3 days", 
        "Rental boost badge highlight",
        "Basic rental analytics"
      ],
      "Standard Boost Plan": [
        "Featured in rental search results",
        "Priority listing for 7 days",
        "Rental boost badge highlight", 
      ...
      "Premium Boost Plan": [
        ...
        "Rental boost badge highlight",
        "Premium rental analytics & insights",
        "Social media promotion",
  ...
  // Define the specific prices for rental boost plans based on plan name
  const getPlanPrice = (planName) => {
    const priceMap = {
      // Regular boost plans (for sale properties)
      "Basic Boost Plan": 30,
      "Standard Boost Plan": 50,
      ...
      "Premium Plan": 180,
      // Rental boost plans
      "Basic Rental Boost": 25,
      "Standard Rental Boost": 45,
      "Premium Rental Boost": 150
    };
    return priceMap[planName] || 30; // Default to 30 if plan name not found
  };

  // Define the specific days for rental boost plans based on plan name
  const getPlanDays = (planName) => {
    const daysMap = {
      // Regular boost plans (for sale properties)
      "Basic Boost Plan": 3,
      "Standard Boost Plan": 7,
      ...
      "Premium Plan": 30,
      // Rental boost plans
      "Basic Rental Boost": 3,
      "Standard Rental Boost": 7,
      "Premium Rental Boost": 30
    };
  ...
  const handleCheckout = async (id) => {
    ...
    const requestData = {
      ...
      success_url: APP_BASE_URL + `/rental-pricing/${propertyId}`,
      cancel_url: APP_BASE_URL + `/rental-pricing/${propertyId}`,
    };
      ...
      requestData,
      {
        ...
        onError: (error) => {
          ...
          if (errorCode === 'MISSING_EMAIL') {
            ...
            alert('❌ This plan cannot be used for boosting rental properties. Please contact support.');
            ...
            alert('❌ You don\'t have permission to boost this rental property. Only the property owner can boost it.');
          } else if (errorMessage.includes('pending_delist') || errorMessage.includes('delisted')) {
            alert('❌ This rental property cannot be boosted because it is delisted or pending deletion. Please relist the property first.');
          } else if (errorMessage.includes('already boosted')) {
            alert('⚠️ This rental property is already boosted. Please wait for the current boost to expire before purchasing a new one.');
          } else if (errorMessage.includes('not approved') || errorMessage.includes('visible to buyers')) {
            alert('❌ This rental property must be approved and visible to tenants before it can be boosted.');
            ...
            let message = '⚠️ There is an incomplete payment for this rental property.\n\n' +
      ...
      { propertyId: propertyId },
      {
        ...
        onError: (error) => {
          ...
          if (error.response?.status === 403) {
            alert("❌ Access denied. You don't have permission to clear payments for this rental property.");
          } else if (error.response?.status === 404) {
            alert("❌ Rental property not found. Please check the URL and try again.");
          } else {
            ...
            >
              ...
              <div className="flex-grow mb-6">
                <ul className="space-y-3">
                  {(getPlanFeatures(item?.name) || item?.features || []).map((feature, featureIndex) => (
