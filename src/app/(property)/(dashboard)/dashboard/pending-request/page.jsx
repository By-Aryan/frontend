...
const DashboardPendingRequestPropertyByRole = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [limit, setLimit] = useState(10);

  const fetchRequestsByStatus = async () => {
    const response = await ApiFetchRequest(
      `/my-filter-properties?page=${currentPage}&limit=${limit}&approval_status=Pending`
    );
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ["pending-requests", currentPage, limit],
...
export default DashboardPendingRequestPropertyByRole;
