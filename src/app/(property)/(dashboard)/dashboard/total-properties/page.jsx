...
const DashboardTotalPropertyByRole = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [limit, setLimit] = useState(10);

  const fetchRequestsByStatus = async () => {
    const response = await ApiFetchRequest(
      `/my-filter-properties?page=${currentPage}&limit=${limit}`
    );
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ["total-properties", currentPage, limit],
...
export default DashboardTotalPropertyByRole;
