...
const DashboardRequestAcceptedPropertyByRole = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [limit, setLimit] = useState(10);

  const fetchRequestsByStatus = async () => {
    const response = await ApiFetchRequest(
      `/my-filter-properties?page=${currentPage}&limit=${limit}&requested_property=true&status=Accepted`
    );
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ["request-accepted", currentPage, limit],
      ...
      <div className="row align-items-center">
        <div className="col-xxl-4">
          <div className="dashboard_title_area">
            <h2>Request Accepted Properties</h2>
...
export default DashboardRequestAcceptedPropertyByRole;
