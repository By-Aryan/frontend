...
const DashboardTotalRequestPropertyByRole = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [limit, setLimit] = useState(10);

  const fetchRequestsByStatus = async () => {
    const response = await ApiFetchRequest(
      `/my-filter-properties?page=${currentPage}&limit=${limit}&excludeStatus=Accepted`
    );
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ["total-requests", currentPage, limit],
      ...
      <div className="row align-items-center">
        <div className="col-xxl-3">
          <div className="dashboard_title_area">
            <h2>Total Requested Properties</h2>
...
export default DashboardTotalRequestPropertyByRole;
